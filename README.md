...
| 상태 저장 | sessionStorage (게임 상태) / localStorage + 쿠키 (인증 토큰·게스트 UUID) |
| 패키지 관리 | pip / venv |
...
3. GitHub 최신 버전 확인 → 업데이트 있으면 자동 적용 후 재실행 요청
4. 서버 시작 후 브라우저 자동 오픈 (`http://localhost:8000`)
...
6. **장 종결** — 핵심 갈등이 새로운 균형에 도달하면 장이 종결되고 요약·총평 제공
7. **다음 장** — 플레이어 입력을 받아 다음 장으로 이어짐
